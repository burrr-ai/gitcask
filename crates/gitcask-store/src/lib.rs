...
//!
//! Every backend exposes exactly the primitives gitcask needs to build
...
//!
//! [`Version`] is opaque to callers: S3/rustfs ETag or a counter in the
//! test-only [`memory::MemoryStore`]. Callers must never parse it.
...
/// Opaque object version (S3 ETag / test counter). Compare only for equality.
...
pub struct ObjectMeta {
    ...
    pub size: u64,
    pub version: Version,
    /// Store-reported object modification time. LIST callers use this for
    /// oldest-first work queues without an extra metadata request per object.
...
pub trait ObjectStore: Send + Sync + 'static {
    /// Human-readable backend id for logs/metrics ("s3", "memory").
    ...
    /// keys under `prefix` (an S3 `delimiter = "/"` listing — so it walks the
    ...
    /// Presigned/authenticated URL for direct client download (for example LFS).
    ...
    /// S3 returns a presigned GET URL (`Range` is not a signed header, so the edge may slice).
    /// Backends without one return `None` and the bytes stream through gitcask.
    async fn accel_target(&self, _key: &str) -> Option<AccelTarget> {
        None
    }

    /// Whether [`ObjectStore::compose`] is available. S3 uses multipart
    /// `UploadPartCopy` (server-side copy, no bytes through this process beyond
    /// one small part).
...
impl MeteredStore {
    fn new(inner: DynStore) -> Self {
        MeteredStore { inner }
    }

    fn record<T>(op: &'static str, result: &Result<T>) {
        let outcome = match result {
            ...
            Err(StoreError::InvalidArgument(_)) => "invalid_argument",
            Err(StoreError::Other(_)) => "error",
        };
        metrics::counter!("gitcask_store_requests_total", "op" => op, "outcome" => outcome)
            .increment(1);
    }
}
...
impl ObjectStore for MeteredStore {
    ...
    async fn get(&self, key: &str, opts: GetOptions) -> Result<GetResult> {
        let result = self.inner.get(key, opts).await;
    ...
    async fn head(&self, key: &str) -> Result<Option<ObjectMeta>> {
        ...
        Self::record("head", &result);
        result
    }

    async fn put(&self, key: &str, body: PutBody, opts: PutOptions) -> Result<ObjectMeta> {
        let result = self.inner.put(key, body, opts).await;
        Self::record("put", &result);
        result
    }

    async fn delete(&self, key: &str, if_version: Option<Version>) -> Result<()> {
        let result = self.inner.delete(key, if_version).await;
    ...
    fn list(
        ...
        metrics::counter!("gitcask_store_requests_total", "op" => "list", "outcome" => "ok")
            .increment(1);
    ...
    async fn list_prefixes(&self, prefix: &str) -> Result<Vec<String>> {
        ...
        Self::record("list_prefixes", &result);
        result
    }
    ...
    async fn compose(
        ...
        let result = self.inner.compose(dest, sources, opts).await;
        Self::record("compose", &result);
        result
    }
}
...
pub async fn open_store(cfg: &gitcask_config::Config) -> anyhow::Result<DynStore> {
    let prefix = cfg.store_prefix();
    let inner: DynStore = match cfg.store.backend {
        gitcask_config::StoreBackend::Memory => {
            #[cfg(feature = "testing")]
            {
            ...
            {
                anyhow::bail!("memory backend requires the `testing` feature")
            }
        }
        gitcask_config::StoreBackend::S3 => {
            #[cfg(feature = "s3")]
            {
    ...
    let inner: DynStore = Arc::new(MeteredStore::new(inner));
    if prefix.is_empty() {
        Ok(inner)
    } else {
