...
/// A process-stable identity: explicit `GITCASK_INSTANCE_NAME`/`GITCASK_INSTANCE_ID`,
...
pub fn instance_id() -> &'static str {
    static ID: LazyLock<String> = LazyLock::new(|| {
        if let (Ok(name), Ok(inst)) = (
            std::env::var("GITCASK_INSTANCE_NAME"),
            std::env::var("GITCASK_INSTANCE_ID"),
        ) && !name.is_empty()
            && !inst.is_empty()
        {
...
mod tests {
    ...
    use gitcask_proto::v1::RepoCatalog;
