...
pub struct MemoryStore {
    objects: Mutex<BTreeMap<String, (Version, Bytes, SystemTime)>>,
    counter: AtomicU64,
    ...
    /// URL + bearer pair used by static-serving tests.
...
impl ObjectStore for MemoryStore {
    ...
    async fn get(&self, key: &str, opts: GetOptions) -> Result<GetResult> {
        self.delay().await;
        let (version, data, last_modified) = {
            let g = self.objects.lock();
            match g.get(key) {
                Some((v, d, last_modified)) => (v.clone(), d.clone(), *last_modified),
    ...
    async fn head(&self, key: &str) -> Result<Option<ObjectMeta>> {
        ...
        Ok(self
            ...
            .lock()
            .get(key)
            .map(|(v, d, last_modified)| ObjectMeta {
                key: key.into(),
                size: d.len() as u64,
                version: v.clone(),
    ...
    async fn put(&self, key: &str, body: PutBody, opts: PutOptions) -> Result<ObjectMeta> {
        ...
        let current = g.get(key).map(|(v, _, _)| v.clone());
        ...
        g.insert(key.to_owned(), (version.clone(), data, last_modified));
        Ok(ObjectMeta {
            key: key.into(),
            size,
            version,
    ...
    async fn delete(&self, key: &str, if_version: Option<Version>) -> Result<()> {
        ...
        match (g.get(key), if_version) {
            ...
            (Some((v, _, _)), Some(want)) if *v != want => Err(StoreError::PreconditionFailed {
                key: key.into(),
                current: Some(v.clone()),
            }),
            _ => {
    ...
    fn list(
        ...
        let items: Vec<ObjectMeta> = g
            ...
            .map(|(k, (v, d, last_modified))| ObjectMeta {
                key: k.clone(),
                size: d.len() as u64,
                version: v.clone(),
                last_modified: *last_modified,
