...
pub struct FaultPlan {
    ...
    /// Deterministically return `Retryable` before the first N in-scope ops.
    /// This models an S3 503 burst without relying on the probabilistic dice.
    pub fail_first: u64,
    ...
    /// Return `Retryable` before the first matching operation, once per pattern.
    /// Supports the same `"op:key"` scope as `panic_once_keys`.
    pub fail_once_keys: Vec<String>,
...
pub struct FaultStore {
    ...
    rng: Mutex<Rng>,
    stats: Stats,
    fail_first_remaining: AtomicU64,
    /// Patterns from `panic_once_keys` already fired.
    fired_panics: Mutex<Vec<String>>,
    /// Patterns from `fail_once_keys` already fired.
    fired_failures: Mutex<Vec<String>>,
...
impl FaultStore {
    pub fn new(inner: DynStore, name: impl Into<String>, seed: u64) -> Arc<Self> {
        Arc::new(FaultStore {
            ...
            stats: Stats::default(),
            fail_first_remaining: AtomicU64::new(0),
            fired_panics: Mutex::new(Vec::new()),
            fired_failures: Mutex::new(Vec::new()),
    ...
    pub fn set(&self, plan: FaultPlan) {
        self.fail_first_remaining
            .store(plan.fail_first, Ordering::Relaxed);
        *self.plan.lock() = plan;
    }
    ...
    pub fn heal(&self) {
        self.fail_first_remaining.store(0, Ordering::Relaxed);
    ...
    fn pattern_matches(pattern: &str, op: &str, key: &str) -> bool {
        match pattern.split_once(':') {
            Some((pattern_op, pattern_key))
                if ["get", "head", "put", "delete", "compose"].contains(&pattern_op) =>
            {
                pattern_op == op && key.contains(pattern_key)
            }
            _ => key.contains(pattern),
        }
    }

    fn take_fail_first(&self) -> bool {
        self.fail_first_remaining
    ...
    async fn decide(
        ...
        if let Some(p) = plan
            .panic_once_keys
            .iter()
            .find(|pattern| Self::pattern_matches(pattern, op, key))
        {
        ...
        if let Some(pattern) = plan
            .fail_once_keys
            .iter()
            .find(|pattern| Self::pattern_matches(pattern, op, key))
        {
            let mut fired = self.fired_failures.lock();
            if !fired.contains(pattern) {
                fired.push(pattern.clone());
                drop(fired);
                self.stats.err_before.fetch_add(1, Ordering::Relaxed);
                self.log(format!("{} {op} {key}: fail-once", self.name));
                return Decision::ErrBefore;
            }
        }
        if plan.black_hole {
        ...
        if self.take_fail_first() {
            self.stats.err_before.fetch_add(1, Ordering::Relaxed);
            self.log(format!("{} {op} {key}: fail-first", self.name));
            return Decision::ErrBefore;
        }
        let (roll, cut) = {
...
impl ObjectStore for FaultStore {
    ...
    fn list(
        ...
        if Self::in_scope(&plan, prefix) && self.take_fail_first() {
            self.stats.err_before.fetch_add(1, Ordering::Relaxed);
            let e = self.retryable("list", prefix, "fail-first");
    ...
    async fn list_prefixes(&self, prefix: &str) -> Result<Vec<String>> {
        ...
        if Self::in_scope(&plan, prefix) && self.take_fail_first() {
            self.stats.err_before.fetch_add(1, Ordering::Relaxed);
            return Err(self.retryable("list_prefixes", prefix, "fail-first"));
        }
        if Self::in_scope(&plan, prefix) && self.rng.lock().f64() < plan.p_err_before {
            self.stats.err_before.fetch_add(1, Ordering::Relaxed);
...
mod tests {
    ...
    #[tokio::test]
    async fn retryable_operations_recover_after_two_failures() {
        let truth: DynStore = MemoryStore::shared();
        let seeded = truth
            .put_bytes("get", b"value".to_vec(), PutMode::Overwrite)
            .await;
        assert!(seeded.is_ok());
        let seeded = truth
            .put_bytes("list/item", b"value".to_vec(), PutMode::Overwrite)
            .await;
        assert!(seeded.is_ok());
        let seeded = truth
            .put_bytes("delete", b"value".to_vec(), PutMode::Overwrite)
            .await;
        assert!(seeded.is_ok());
        let link = FaultStore::new(truth.clone(), "retry", 7);
        ...
        let result =
            util::with_retry("get", "get", 4, || link.get("get", GetOptions::default())).await;
        assert!(result.is_ok());
        errors += 2;
        assert_eq!(link.stats().err_before.load(Ordering::Relaxed), errors);

        link.set(FaultPlan {
            fail_first: 2,
            ..Default::default()
        });
        let result = util::with_retry("head", "get", 4, || link.head("get")).await;
        ...
        let result = util::with_retry("list", "list/", 4, || async {
            let mut objects = link.list("list/", None);
            match objects.next().await {
                Some(result) => result,
                None => Err(StoreError::other(anyhow::anyhow!(
                    "fault test list returned no object"
                ))),
            }
        })
        .await;
        ...
        let result = util::with_retry("delete", "delete", 4, || link.delete("delete", None)).await;
        assert!(result.is_ok());
        errors += 2;
        assert_eq!(link.stats().err_before.load(Ordering::Relaxed), errors);
        ...
        let result = util::with_retry("put", "put", 4, || {
            link.put(
                "put",
                PutBody::Bytes(Bytes::from_static(b"value")),
    ...
    #[tokio::test]
    async fn create_put_returns_retryable_without_retrying() {
        let truth: DynStore = MemoryStore::shared();
        let link = FaultStore::new(truth, "create", 11);
        link.set(FaultPlan {
            fail_first: 1,
            ..Default::default()
        });
        ...
        assert!(matches!(result, Err(StoreError::Retryable(_))));
        assert_eq!(link.stats().ops.load(Ordering::Relaxed), 1);
    }

    #[tokio::test]
    async fn exhausted_retries_preserve_retryable_error() {
        let truth: DynStore = MemoryStore::shared();
        let link = FaultStore::new(truth, "exhausted", 13);
        link.set(FaultPlan {
            fail_first: 3,
            ..Default::default()
        });

        let result = util::with_retry("head", "key", 2, || link.head("key")).await;

        assert!(matches!(result, Err(StoreError::Retryable(_))));
        assert_eq!(link.stats().ops.load(Ordering::Relaxed), 3);
    }

    #[tokio::test]
    async fn precondition_failure_is_not_retried() {
        let truth: DynStore = MemoryStore::shared();
        let link = FaultStore::new(truth, "precondition", 17);
        link.set(FaultPlan {
            p_cas_fail: 1.0,
            ..Default::default()
        });

        let result = util::with_retry("put", "key", 4, || {
            link.put(
                "key",
                PutBody::Bytes(Bytes::from_static(b"value")),
                PutOptions::from(PutMode::Create),
