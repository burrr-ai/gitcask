//! Generated protobuf types for gitcask's on-store formats.
//!
//! Schema lives in `proto/gitcask/v1/wal.proto`; it is the contract between
//! every gitcask instance and must only evolve backward-compatibly.

pub mod v1 {
    include!(concat!(env!("OUT_DIR"), "/gitcask.v1.rs"));
}
...
pub mod keys {
    ...
    /// Shared immutable JSON render cache, relative to a repository prefix.
    pub const API_CACHE_DIR: &str = "cache/api/v1/";
    /// Shared immutable archive cache, relative to a repository prefix.
    pub const ARCHIVE_CACHE_DIR: &str = "cache/archive/v1/";
    ...
    /// Bucket-root prefix of repositories awaiting maintenance.
    pub const PENDING_DIR: &str = "pending/";
    pub fn pending_key(owner: &str, name: &str) -> String {
        format!("{PENDING_DIR}{owner}/{name}")
    }
    ...
    /// Last completed repository-bucket GC cursor (`GcState`).
...
mod tests {
    ...
    fn keys() {
        ...
        assert_eq!(keys::pending_key("acme", "repo"), "pending/acme/repo");
        assert_eq!(
