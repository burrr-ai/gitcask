...
pub async fn parse<R: AsyncRead + Unpin>(
    ...
    GitError,
> {
    let mut updates: Vec<gitcask_proto::v1::RefUpdate> = Vec::new();
...
fn parse_command_line(b: &[u8]) -> Result<(gitcask_proto::v1::RefUpdate, String), GitError> {
...
pub fn txn_from_snapshot(snap: &RefSnapshotData) -> gitcask_proto::v1::RefTransaction {
    let mut updates: Vec<gitcask_proto::v1::RefUpdate> = snap
        .refs
        .iter()
        .map(|r| gitcask_proto::v1::RefUpdate {
            name: r.name.clone(),
            old_oid: String::new(),
            new_oid: r.oid.clone(),
            new_symbolic_target: String::new(),
