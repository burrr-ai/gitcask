...
use gitcask_git::pkt::Protocol;
use gitcask_git::{LocalRepo, ObjectFormat, RepoId, UploadPackRequest, gix_hash};
...
async fn run_fetch(repo: &LocalRepo, req: &UploadPackRequest) -> Vec<u8> {
...
async fn fetch_no_haves_produces_valid_pack() {
    {
        ...
        let resp = run_fetch(&repo, &req).await;
        ...
        assert!(pack.starts_with(b"PACK"), "packfile missing");
        ...
        let out = std::process::Command::new("git")
            ...
            .output()
            .unwrap();
        assert!(out.status.success(), "B missing from fetched pack");
    }
}
...
async fn fetch_with_haves_smaller_pack_and_ack() {
    {
        ...
        let ack = run_fetch(&repo, &req_ack).await;
        assert!(cm::has_ack(&ack, &a), "expected ACK {a}");
        let full = run_fetch(&repo, &req_full).await;
        let inc = run_fetch(&repo, &req_inc).await;
        let pack_full = cm::extract_packfile(&full);
        let pack_inc = cm::extract_packfile(&inc);
        assert!(
            pack_inc.len() < pack_full.len(),
            "incremental not smaller: {} vs {}",
            pack_inc.len(),
            pack_full.len()
        );
...
async fn fetch_filter_blob_none_no_blobs() {
    {
        ...
        assert_eq!(blobs, 0, "filter blob:none produced blobs");
        assert!(commits >= 1, "no commits in filtered pack");
    }
}
...
async fn fetch_filter_blob_limit_excludes_large_blobs() {
    {
        ...
        assert_eq!(
            blobs, 2,
            "blob:limit=100 should exclude 5000-byte blob: got {blobs}"
        );
        assert!(commits >= 1, "no commits");
    }
}
...
async fn fetch_filter_tree_0_only_root_tree() {
    {
        ...
        assert_eq!(blobs, 0, "tree:0 should exclude blobs");
        assert!(commits >= 1, "no commits");
        assert_eq!(trees, 0, "tree:0 must not send trees");
    }
}
...
async fn fetch_include_tag_sends_annotated_tag() {
    {
        ...
        assert!(
            tags >= 1,
            "include-tag should include annotated tag object: got {tags} tags"
        );
    }
}
...
async fn fetch_deepen_shallow_info() {
    {
        ...
        let resp = run_fetch(&repo, &req).await;
        // The response should contain shallow-info.
        let resp_str = String::from_utf8_lossy(&resp);
        assert!(
            resp_str.contains("shallow-info") || resp_str.contains("shallow "),
            "shallow-info section missing"
        );
        ...
        assert!(
            shallow_commits <= 2,
            "deepen=2 should produce at most 2 commits: got {shallow_commits}"
        );
    }
}
...
async fn fetch_haves_shrink_pack() {
    {
        ...
        assert!(
            pack_inc.len() < pack_full.len(),
            "incremental pack should be smaller than full pack: {} vs {}",
            pack_inc.len(),
            pack_full.len()
        );
...
async fn fetch_fsck_rejects_corrupt_pack() {
    ...
    // test infrastructure, not upload-pack itself).
    let (_r, repo, _a, b) = setup().await;
    let req = UploadPackRequest {
        wants: vec![oid(&b)],
        haves: vec![],
...
/// repository. Upload-pack must not try to pack it (it would fail the object
/// lookup) under any filter, and the resulting pack must still be
