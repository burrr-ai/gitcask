...
use gitcask_git::{LocalRepo, LsRefsArgs, ObjectFormat, RepoId, gix_hash};
use gitcask_proto::v1::{Ref, RefSnapshot};
...
fn bench_ls_refs_466k() {
    ...
    let start = Instant::now();
    repo.advertise_refs_v0(gitcask_git::Service::UploadPack, &mut advert)
        .unwrap();
