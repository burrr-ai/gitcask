...
use gitcask_git::{LocalRepo, ObjectFormat, RefSnapshotData, RepoId, gix_hash};
use gitcask_proto::v1::{Ref, RefSnapshot, RefTransaction, RefUpdate};
...
async fn ref_txn_atomicity_and_conflicts() {
    ...
    let err = repo
        ...
        .unwrap_err();
    assert!(
        matches!(err, gitcask_git::GitError::RefConflict { ref name, .. } if name == "refs/heads/main")
    );
    ...
    let err = repo
        ...
        .unwrap_err();
    assert!(matches!(err, gitcask_git::GitError::RefConflict { .. }));
...
fn ref_view_lookups_are_logarithmic_and_overlay_aware() {
    use gitcask_git::{Ref, RefSnapshotData, RefView};
    let n = 466_395u32;
