...
//! only a wait that actually queued is timed, recorded in `gitcask_lock_wait_seconds{lock}`, kept
...
pub fn record(
    lock: &'static str,
    repo: &gitcask_git::RepoId,
    waited: Duration,
    warn_after: Duration,
) {
    metrics::histogram!("gitcask_lock_wait_seconds", "lock" => lock).record(waited.as_secs_f64());
...
pub fn snapshot_for(repo: &gitcask_git::RepoId) -> Vec<(&'static str, u64)> {
    let key = repo.to_string();
    PER_REPO
        .lock()
        .iter()
