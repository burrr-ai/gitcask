...
//! (pack materialization, compaction,
//! fsck, ...). Every task
//!
//! * gets a unique id and a record in a per-instance log (bounded per repo and
//!   across the instance),
...
//! mutating tasks is still the store lease (compaction); this registry
...
const KEEP_RECORDS_TOTAL: usize = 10_000;
...
#[derive(Default)]
struct TaskHistory {
    recent: HashMap<String, VecDeque<Arc<TaskState>>>,
    by_id: HashMap<String, Arc<TaskState>>,
    repo_order: VecDeque<String>,
}
...
impl Tasks {
    ...
    pub fn begin(
        ...
        drop(running);
        {
            ...
            history.by_id.insert(record.id.clone(), state.clone());
            history.repo_order.retain(|known| known != repo);
            history.repo_order.push_back(repo.to_string());
            let old = {
                let q = history.recent.entry(repo.to_string()).or_default();
                q.push_back(state.clone());
                if q.len() > KEEP_RECORDS {
                    q.pop_front()
                } else {
                    None
                }
            ...
            if let Some(old) = old {
                history.by_id.remove(&old.id());
            }
            while history.by_id.len() > KEEP_RECORDS_TOTAL {
                let Some(oldest_repo) = history.repo_order.pop_front() else {
                    break;
                };
                if let Some(records) = history.recent.remove(&oldest_repo) {
                    for old in records {
                        history.by_id.remove(&old.id());
                    }
                }
        ...
        metrics::counter!("gitcask_tasks_started_total", "kind" => kind.to_string()).increment(1);
    ...
    fn finish(
        ...
        *state.outcome.lock() = Some(out);
        ...
        // (materialize, compaction) — dashboards key on them.
        let pick = |k: &str| {
            pick_from
                .as_ref()
                .and_then(|v| v.get(k))
        ...
        metrics::counter!("gitcask_tasks_finished_total", "kind" => record.kind.clone(), "ok" => ok.to_string()).increment(1);
        metrics::histogram!("gitcask_task_duration_seconds", "kind" => record.kind.clone(), "ok" => ok.to_string()).record(record.elapsed_ms as f64 / 1000.0);
        record
    }

    pub fn get(&self, id: &str) -> Option<Arc<TaskState>> {
        self.history.lock().by_id.get(id).cloned()
    }
...
mod tests {
    ...
    #[test]
    fn task_history_has_a_global_record_bound() {
        let tasks = Tasks::new();
        let mut oldest_id = None;
        let mut newest_id = None;
        for index in 0..=KEEP_RECORDS_TOTAL {
            let repo = format!("bounded/repo-{index}");
            let Begin::Started(task) = tasks.begin(&repo, "test", HashMap::new(), None) else {
                panic!("a unique repo cannot already have a running task");
            };
            let id = task.id();
            if index == 0 {
            ...
            newest_id = Some(id);
            task.finish_ok("done", None);
        }

        let history = tasks.history.lock();
        let recent_records: usize = history.recent.values().map(VecDeque::len).sum();
        assert!(history.by_id.len() <= KEEP_RECORDS_TOTAL);
        assert_eq!(recent_records, history.by_id.len());
        assert!(!history.by_id.contains_key(&oldest_id.unwrap()));
        assert!(history.by_id.contains_key(&newest_id.unwrap()));
    }
}
