...
use gitcask_proto::time;
use gitcask_proto::v1::{Checkpoint, CheckpointRef, Manifest, RefSnapshot};
...
pub(crate) async fn write_checkpoint_impl(handle: &RepoHandle) -> Result<CheckpointRef, WalError> {
    ...
    match &r {
        Ok(cp) => {
            ...
            metrics::histogram!("gitcask_checkpoint_seconds").record(t0.elapsed().as_secs_f64());
            metrics::counter!("gitcask_checkpoints_total", "outcome" => "ok").increment(1);
        }
        Err(_) => {
            span.record("outcome", "error");
            metrics::counter!("gitcask_checkpoints_total", "outcome" => "error").increment(1);
        }
    }
    r
}
...
async fn write_checkpoint_inner(handle: &RepoHandle) -> Result<CheckpointRef, WalError> {
    ...
    // flags travel in PackRef) + the ref snapshot.
