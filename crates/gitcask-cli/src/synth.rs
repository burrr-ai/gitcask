//! `gitcask synth` — generate a deterministic synthetic git repository.
//!
...
pub async fn run(
    ...
    for (key, val) in [
        ("user.name", "gitcask-synth"),
        ("user.email", "synth@gitcask.local"),
...
fn generate_stream(
    ...
    for commit_idx in 0..n_commits {
        ...
        w.write_str(&format!(
            "author gitcask-synth <synth@gitcask.local> {ts_str}\n"
        ));
        w.write_str(&format!(
            "committer gitcask-synth <synth@gitcask.local> {ts_str}\n"
        ));
        ...
        for (tp, name) in &tag_points {
            if *tp == commit_num {
                ...
                w.write_str(&format!("from :{commit_mark}\n"));
                w.write_str(&format!(
                    "tagger gitcask-synth <synth@gitcask.local> {ts_str}\n"
                ));
                w.write_str(&format!("data {}\n", tag_msg.len() + 1));
