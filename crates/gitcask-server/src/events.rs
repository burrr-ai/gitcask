...
//! dedup key = `(repo, _gitcask.seq, ref_name)`; order by `_gitcask.seq`.

use gitcask_git::RepoId;
use gitcask_proto::v1::{EntryKind, LogEntry};
use serde::Serialize;
...
/// Top level byte-compatible with the common `RefEvent` shape; gitcask adds `repo`
/// and `_gitcask`.
#[derive(Debug, Clone, Serialize)]
pub struct RefEvent {
    pub action: Action,
    ...
    pub repo: String,
    #[serde(rename = "_gitcask")]
    pub gitcask: GitcaskExt,
}
...
impl RefEvent {
    ...
    /// Transport headers (`x_gitcask_<name>`): filtering without parsing bodies.
...
/// `POST`ed with `Content-Type: application/json`, a `X-Gitcask-Delivery` id
/// (sha1 of the body — the consumer's dedup key) and, when `events.webhook_secret`
/// is set, `X-Gitcask-Signature: sha256=<hex HMAC-SHA256 of the body>`. A non-2xx
...
impl Sink for WebhookSink {
    ...
    async fn deliver(&self, batch: &[RefEvent]) -> anyhow::Result<()> {
        ...
        let mut req = self
            ...
            .header("x-gitcask-delivery", delivery);
        if let Some(secret) = &self.secret {
            req = req.header("x-gitcask-signature", Self::signature(secret, &body));
        }
...
mod tests {
    ...
    fn ref_event_golden() {
        ...
        let want = concat!(
            ...
            "\"_gitcask\":{\"schema_version\":1,\"seq\":\"42\",\"entry_kind\":\"push\",",
            "\"request_id\":\"d1f916f7\"}}"
        );
        assert_eq!(got, want);
    ...
    fn log_entry(kind: EntryKind, name: &str, old: &str, new: &str, sym: &str) -> LogEntry {
        ...
        LogEntry {
            seq: 9,
            kind: kind as i32,
            txn: Some(gitcask_proto::v1::RefTransaction {
    ...
    fn conversion_emits_ref_skips_compact_noop_symbolic() {
        let repo = gitcask_git::RepoId::new("t", "r").unwrap();
        let mut out = Vec::new();
        refs_from_entries(
            &repo,
            &[
        ...
        assert_eq!((r.repo.as_str(), r.gitcask.seq.as_str()), ("t/r", "9"));
        assert_eq!(r.gitcask.entry_kind, "push");
        let d = &out[1];
