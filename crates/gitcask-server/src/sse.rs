//! The SSE envelope: any JSON endpoint whose answer needs
...
//! ```
//!
//! plus `: keepalive` comments. Packets come from [`gitcask_wal::Progress`]
...
use gitcask_wal::Progress;
use gitcask_wal::progress::ProgressRx;
use serde::Serialize;
    ...
    F: Future<Output = Result<Rendered, ApiError>> + Send + 'static,
{
    ...
    let head =
        futures::stream::once(async { Ok::<_, Infallible>(Bytes::from_static(b": gitcask\n\n")) });
...
pub fn task_stream(state: std::sync::Arc<gitcask_wal::tasks::TaskState>) -> Response {
    ...
    tokio::spawn(async move {
        ...
        let send_outcome = |o: Result<gitcask_wal::tasks::TaskOutcome, (u16, String)>| match o {
