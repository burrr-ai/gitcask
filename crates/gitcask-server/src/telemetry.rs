...
//!   (background tasks, materialization, compaction) so one job groups in the UI.
//!
...
use gitcask_config::{Config, LogFormat};
...
struct SpanData {
    ...
    /// close while any child span is alive, and SDK-internal spans
    /// under `store.put` linger until the stream idles (~3 min) — prod
    /// 2026-08-21: a 1.2 s bulk upload logged `elapsed_ms = 173700`, and
...
impl CloudLoggingLayer {
    ...
    fn base_record(&self, severity: &str, message: &str) -> Map<String, Value> {
        ...
        if let Ok(name) = std::env::var("GITCASK_INSTANCE_NAME") {
...
/// incoming trace: background tasks, materialization, compaction, publisher).
...
mod tests {
    ...
    /// A span whose close is deferred by a lingering child (an SDK's
