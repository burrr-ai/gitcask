//! HTTP serving of immutable store objects (LFS objects and packs) with
//! the complete conditional/range contract a CDN or `git` expects:
//!
//! * strong `ETag` = the store version (S3 ETag), quoted;
...
use std::ops::Range;
use std::{io::SeekFrom, time::UNIX_EPOCH};

use axum::body::Body;
use axum::http::{HeaderMap, HeaderValue, Method, StatusCode, header};
use axum::response::{IntoResponse, Response};
use gitcask_store::{GetOptions, GetResult, ObjectMeta, ObjectStore, StoreError, Version};
...
pub const CAPABILITIES_HEADER: &str = "x-gitcask-capabilities";
/// Capability: the edge honours `X-Accel-Redirect: /_store/` by fetching the object from
/// the bucket itself — at the URL in `X-Gitcask-Store-Url`, with `X-Gitcask-Store-Authorization`
/// when present (S3 uses a presigned URL and no header) — slicing ranges and caching
/// the bytes on its disk under `X-Gitcask-Store-Key`.
pub const CAP_ACCEL_REDIRECT: &str = "accel-redirect";
/// Internal nginx location that proxies to `X-Gitcask-Store-Url`.
...
pub struct ServeOptions<'a> {
    ...
    /// When the request carries `X-Gitcask-Capabilities: accel-redirect` and the store
    ...
    /// header and steal `X-Gitcask-Store-Authorization`.
...
pub async fn serve(
    ...
    // caches the bytes on its disk, so a large object never ties up a worker on
        ...
        && opts.peer.is_some_and(|p| p.ip().is_loopback())
    {
        if let Some(target) = store.accel_target(key).await {
            ...
            h.insert("x-gitcask-store-url", hv(&target.url)?);
            if let Some(auth) = &target.authorization {
                h.insert("x-gitcask-store-authorization", hv(auth)?);
            }
            // The edge's cache key: the object, not the (possibly presigned, changing) URL.
            h.insert(
                "x-gitcask-store-key",
                hv(&gitcask_store::util::encode_path(key))?,
            );
            h.insert("x-gitcask-accel", HeaderValue::from_static(store.backend()));
            ...
            h.insert("x-gitcask-etag", etag_of(&meta.version));
            return Ok(resp);
        }
    }
    ...
    // One tag maps onto the store's conditional read. Lists / `*` are compared
    // after the read; the body is dropped.
    let tags = etags(headers, header::IF_NONE_MATCH);
    let get = GetOptions {
...
/// Serve an already-open immutable local file with the same validators,
/// ranges, and download headers as [`serve`]. The caller supplies a stable
/// strong `etag` for the file's immutable inputs. The file is streamed and
/// may already be unlinked from the local cache.
pub async fn serve_file(
    ...
    let meta = ObjectMeta {
        key: String::new(),
        size,
        version: Version::new(etag.trim_matches('"')),
    ...
    if *method == Method::HEAD {
        ...
        base_headers(&mut response, &meta, &opts);
        response
            .headers_mut()
    ...
    let range = requested.filter(|_| if_range_allows(headers, &meta.version));
    ...
    if let Some(spec) = range {
        ...
        file.seek(SeekFrom::Start(range.start))
            .await
            .map_err(|error| ApiError::Internal(format!("seeking static file: {error}")))?;
        let length = range.end - range.start;
        let body = crate::stream::body_from_async_read(file.take(length));
        let mut response = (StatusCode::PARTIAL_CONTENT, body).into_response();
        base_headers(&mut response, &meta, &opts);
        response.headers_mut().insert(
            header::CONTENT_RANGE,
            HeaderValue::from_str(&format!("bytes {}-{}/{}", range.start, range.end - 1, size))
                .map_err(|error| ApiError::Internal(format!("content range: {error}")))?,
        );
        response
            .headers_mut()
    ...
    file.seek(SeekFrom::Start(0))
        .await
        .map_err(|error| ApiError::Internal(format!("seeking static file: {error}")))?;
    let body = crate::stream::body_from_async_read(file.take(size));
