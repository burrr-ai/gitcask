//! `/api/v1` (D20): the versioned programmatic surface, its repo-scoped browser lane,
//! CORS for foreign origins, discovery, repo summary and admin, and the SDK artefact route.
...
async fn v1_surface_and_browser_lane() -> TestResult {
    ...
    assert!(d["base"].as_str().unwrap().ends_with("/api/v1"));
    assert!(d.get("docs").is_none());
    assert!(
        d["endpoints"]
            .as_array()
            .unwrap()
            .iter()
            .all(|endpoint| !endpoint.as_str().unwrap().contains("merge-queue"))
    );
    ...
    // repo admin under the repo prefix
...
async fn discovery_requires_a_forwarded_principal() -> TestResult {
    ...
    assert_eq!(
        req(&server, reqwest::Method::GET, "/api/v1", &[]).await?.0,
        401
    );
    assert_eq!(
        req(
...
/// D26/D27: everything of a repository under its own prefix, with the same
/// surface under the browser lane `/{o}/{r}/api-browser/…`.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
...
async fn repository_delete_requires_admin() -> TestResult {
    let server = Server::start_with_tweak(|c| {
        c.server.auth_mode = gitcask_config::AuthMode::Forwarded;
    })
    .await?;

    let writer = [("X-Gitcask-Principal", "writer"), ("X-Gitcask-Write", "1")];
    let admin = [
        ("X-Gitcask-Principal", "admin"),
        ("X-Gitcask-Write", "1"),
        ("X-Gitcask-Admin", "1"),
    ];
