...
async fn after_sigterm_new_object_work_is_refused_and_no_unit_starts() -> anyhow::Result<()> {
    let server = Server::start_with_tweak(|c| {
        c.server.roles = vec![gitcask_config::Role::Serve, gitcask_config::Role::Maintain];
    ...
    // A long-running unit: a task of an op kind
    ...
    let unit = match h0.begin_task("compact", Default::default()) {
        ...
        gitcask_wal::Begin::AlreadyRunning(_) => anyhow::bail!("compact already running"),
    };
    ...
    let interrupted = server
        ...
        .registry
        .tasks()
        .interrupt_where(gitcask_server::ops::is_op);
    assert_eq!(
        interrupted,
