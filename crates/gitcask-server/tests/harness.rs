#![allow(dead_code)]
//! Test harness: spin up gitcask-server on a random port backed by the in-memory
...
use gitcask_config::{Config, StoreBackend};
...
pub struct Server {
    ...
    pub state: Arc<gitcask_server::AppState>,
    cache_evictor: Option<tokio::task::JoinHandle<()>>,
...
impl Server {
    ...
    pub async fn start() -> Result<Self> {
        ...
        {
            let mut tcfg = gitcask_config::Config::default();
            tcfg.telemetry.log_format = gitcask_config::LogFormat::Json;
            tcfg.telemetry.log_filter = "warn,gitcask=info,gitcask_server=debug,gitcask_wal=debug,gitcask_git=debug,gitcask_store=debug".into();
            gitcask_server::telemetry::tracing_init(&tcfg);
        }
    ...
    /// Fresh store whose rendered absolute URLs point at an external gateway.
    pub async fn start_with_public_url(
        public_url: String,
        tweak: impl FnOnce(&mut Config),
    ) -> Result<Self> {
        Self::start_with_cfg_and_public_url(
            MemoryStore::shared(),
            tempfile::tempdir()?,
            Some(public_url),
            tweak,
    ...
    /// Second instance on the same store with a tweaked config.
    ...
    async fn start_with_cfg(
        ...
        Self::start_with_cfg_and_public_url(store, cache, None, tweak).await
    }

    async fn start_with_cfg_and_public_url(
        ...
        cfg.server.public_url = Some(public_url.unwrap_or_else(|| base_url.clone()));
        ...
        gitcask_server::bridge::spawn_sweeper(state.clone());
        let cache_evictor = gitcask_server::spawn_cache_evictor(&state);
    ...
    pub async fn read_log(
        ...
        owner: &str,
        repo: &str,
    ) -> Result<Vec<gitcask_proto::v1::LogEntry>> {
    ...
    pub async fn registry_has_packs(&self, owner: &str, repo: &str) -> bool {
        let id = gitcask_git::RepoId::new(owner, repo).unwrap();
...
impl Drop for Server {
    fn drop(&mut self) {
        if let Some(handle) = &self.cache_evictor {
            handle.abort();
        }
    }
}
...
impl TestRepo {
    ...
    pub fn synthetic(commits: usize, files: usize) -> Result<Self> {
        ...
        git_in(&dir, &["init", "-q"])?;
        git_in(&dir, &["config", "user.email", "test@gitcask"])?;
        git_in(&dir, &["config", "user.name", "gitcask test"])?;
        ...
        for c in 0..commits {
            ...
            stream.extend_from_slice(
                format!("author gitcask test <test@gitcask> {timestamp} +0000\n").as_bytes(),
            );
            stream.extend_from_slice(
                format!("committer gitcask test <test@gitcask> {timestamp} +0000\n").as_bytes(),
            );
