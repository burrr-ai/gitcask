#!/usr/bin/env bash
# tests/e2e.sh — end-to-end test: real git vs. gitcask server.
#
# Phase 1 (memory backend): build release, start `gitcask serve` with a generated
# config on a random port, then exercise the full git surface:
#   synth → PUT repo → push → clone → fetch → ls-remote → partial clone →
#   tag → delete → git fsck.
#
# Phase 2 (S3/rustfs, when GITCASK_TEST_S3_ENDPOINT is set): run two instances
# backed by the same rustfs bucket; push to A, clone from B; assert both see
# the same refs and pass fsck.
#
# Usage:
#   tests/e2e.sh
#   GITCASK_TEST_S3_ENDPOINT=http://localhost:9000 tests/e2e.sh
#
...
GITCASK="$ROOT/target/release/gitcask"
...
step "e2e: gitcask end-to-end test"

if [[ ! -x "$GITCASK" ]]; then
    step "building gitcask (release)..."
    (cd "$ROOT" && cargo build -p gitcask-cli --release)
fi
...
EOF
    ...
    step "starting gitcask serve (memory)..."
    RUST_LOG=info "$GITCASK" --config "$TMP/gitcask.toml" serve > "$TMP/server-mem.log" 2>&1 &
    SERVER_PID=$!
    PIDS+=("$SERVER_PID")
    sleep 2
...
SYNTH_DIR="$TMP/synth"
"$GITCASK" --config "$TMP/gitcask.toml" synth --out "$SYNTH_DIR" --size s --seed 12345
...
HTTP_CODE=$(curl -sf -X PUT "$BASE_URL/$REPO.git" -w '%{http_code}' -o /dev/null || true)
...
step "push: clone synth → push to gitcask"
WORK="$TMP/work"
...
(cd "$WORK" && git push -q origin main) || fail "push main failed"
pass "push main"

# --- clone -------------------------------------------------------------------

step "clone: from gitcask"
CLONE="$TMP/clone"
git clone -q "$BASE_URL/$REPO.git" "$CLONE" || fail "clone failed"
pass "clone"
...
(cd "$WORK" && git push -q origin main) || fail "second push failed"
(cd "$CLONE" && git fetch -q origin) || fail "fetch failed"
...
step "ls-remote"
LS=$(git ls-remote "$BASE_URL/$REPO.git" refs/heads/main 2>/dev/null | awk '{print $1}')
...
PARTIAL="$TMP/partial"
git clone -q --filter=blob:none "$BASE_URL/$REPO.git" "$PARTIAL" || fail "partial clone failed"
pass "partial clone"
...
(cd "$WORK" && git push -q origin v1.0.0) || fail "tag push failed"
pass "tag pushed"

TAG_LS=$(git ls-remote "$BASE_URL/$REPO.git" refs/tags/v1.0.0 2>/dev/null | awk '{print $1}')
...
(cd "$WORK" && git push -q origin :refs/tags/v1.0.0) || fail "tag delete failed"
TAG_GONE=$(git ls-remote "$BASE_URL/$REPO.git" refs/tags/v1.0.0 2>/dev/null | wc -l)
...
step "DELETE repo"
HTTP_CODE=$(curl -sf -X DELETE "$BASE_URL/$REPO.git" -w '%{http_code}' -o /dev/null || true)
...
if [[ -z "${GITCASK_E2E_BASE_URL:-}" ]]; then
    kill "$SERVER_PID" 2>/dev/null || true
fi

# S3 phase only makes sense for local testing, not remote (a serverless host) mode.
if [[ -z "${GITCASK_E2E_BASE_URL:-}" && -n "${GITCASK_TEST_S3_ENDPOINT:-}" ]]; then
    step "phase 2: S3 backend ($GITCASK_TEST_S3_ENDPOINT)"

    BUCKET="${GITCASK_TEST_BUCKET:-gitcask-test}"
    PORT_A="$(rand_port)"
    PORT_B="$(rand_port)"
...
bucket = "$BUCKET"
[store.s3]
endpoint = "$GITCASK_TEST_S3_ENDPOINT"
region = "us-east-1"
...
EOF
    ...
    # Start instance A
    RUST_LOG=info env "GITCASK__SERVER__LISTEN=127.0.0.1:${PORT_A}" "GITCASK__CACHE__DIR=$TMP/cache-s3-a" \
        "$GITCASK" --config "$TMP/cfg-s3.toml" serve > "$TMP/server-a.log" 2>&1 &
    SERVER_A_PID=$!
    PIDS+=("$SERVER_A_PID")
    sleep 2
    ...
    # Start instance B
    RUST_LOG=info env "GITCASK__SERVER__LISTEN=127.0.0.1:${PORT_B}" "GITCASK__CACHE__DIR=$TMP/cache-s3-b" \
        "$GITCASK" --config "$TMP/cfg-s3.toml" serve > "$TMP/server-b.log" 2>&1 &
    SERVER_B_PID=$!
    PIDS+=("$SERVER_B_PID")
    sleep 2
    ...
    curl -sf -X PUT "$BASE_A/$REPO2.git" -o /dev/null || fail "create on A"
    pass "repo created on A"

    step "S3: push to A"
    git clone -q "$SYNTH_DIR" "$TMP/work-s3"
    (cd "$TMP/work-s3" && git remote set-url origin "$BASE_A/$REPO2.git")
    (cd "$TMP/work-s3" && git push -q origin main) || fail "push to A"
    pass "pushed to A"

    step "S3: clone from B (cross-instance consistency)"
    git clone -q "$BASE_B/$REPO2.git" "$TMP/clone-s3" || fail "clone from B"
    ...
    curl -s -X DELETE "$BASE_A/$REPO2.git" -o /dev/null || true
